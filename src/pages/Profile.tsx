...
const Profile = () => {
  ...
  const loadProfile = async (profileUserId: string, currentUserId: string) => {
    setIsLoading(true);
    const [profile, userPosts, isFollowingUser, counts] = await Promise.all([getUserProfile(profileUserId), getUserPosts(profileUserId, currentUserId), isFollowing(currentUserId, profileUserId), getFollowCounts(profileUserId)]);
  ...
  const handleFollowToggle = async () => {
    ...
    const success = following ? await unfollowUser(currentUser.id, userId) : await followUser(currentUser.id, userId);
    if (success) {
      ...
      toast({
        ...
        description: following ? `You unfollowed ${profileUser?.displayName}` : `You are now following ${profileUser?.displayName}`
      });
    }
  };
  ...
  return <div className="min-h-screen bg-background pb-20">
      {/* Top Navigation */}
      <div className="sticky top-0 z-10 h-14 items-center justify-between px-4 border-b border-border bg-card/95 backdrop-blur pt-safe py-[2px] flex flex-row">
      ...
      <div className="p-6 border-b border-border">
        <div className="flex items-start gap-6 mb-6">
          ...
          <div className="flex-1">
            <h2 className="font-bold mb-1 text-xl">{profileUser.displayName}</h2>
            <p className="text-muted-foreground mb-3 text-sm">@{profileUser.username}</p>
            ...
            {isOwnProfile ? <UserProfile /> : <Button onClick={handleFollowToggle} variant={following ? "outline" : "default"} className="gap-2">
                {following ? <>
      ...
      <div className="p-4">
        {isLoading ? <div className="text-center py-12">
            ...
            {posts.map(post => <div key={post.id} className="aspect-square bg-muted cursor-pointer hover:opacity-75 transition-opacity" onClick={() => {
          // Could open a modal with full post here
        }}>
                {post.image_url ? <img src={post.image_url} alt="Post" className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center p-2">
